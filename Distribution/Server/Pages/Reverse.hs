  ...
  where
    ...
    reversePackageRender pkgid packageLink isRecent (ReversePageRender renders counts total) =
      ...
      where
        ...
        reverseTable = thediv << table ! [theclass "fancy"] << reverseTableRows
        reverseTableRows =
    ...
    reverseFlatRender pkgname packageLink  (ReverseCount total flat _) pairs =
      ...
      where
        ...
        reverseTableRows =
            ...
            [ tr ! [theclass (if odd n then "odd" else "even")] <<
                [ td << toPackage pkg
                , td ! [theclass "center"] << [ toHtml $ (show count) ++ " (", anchor ! [href $ reverseAllUri reverseResource "" pkg] << "view", toHtml ")" ]
                ]
      ...
      where
        ...
        versionTableRows =
            ...
            [ tr ! [theclass (if odd n then "odd" else "even")] <<
                ...
                , td ! [theclass "center"] << [ toHtml $ show (Map.findWithDefault 0 version versions) ++ " ("
      ...
      where
        ...
        reverseTableRows =
            ...
            [ tr ! [theclass (if odd n then "odd" else "even")] <<
                ...
                , td ! [theclass "center"] << [ toHtml $ show flat ++ " (", anchor ! [href $ reverseStatsUri reverseResource "" pkgname ] << "view", toHtml ")" ]
                , td ! [theclass "center"] << [ toHtml $ show total ++ " (", anchor ! [href $ reverseNameUri reverseResource "" pkgname ] << "view", toHtml ")" ]
